...
async function showOneLine(element){
    ...
    for (index in allSta){
        ...
        try{
            ...
            if(jsData.data[direction].hasOwnProperty('DOWN')){
                let oneData = jsData.data[direction].DOWN
                let time1 =  oneData[0].time
                let time2 =  oneData[1].time
                let plat1 =  oneData[0].plat
                let plat2 =  oneData[1].plat
